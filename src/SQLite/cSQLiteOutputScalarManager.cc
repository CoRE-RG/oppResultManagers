...
void cSQLiteOutputScalarManager::startRun()
{
    ...
    if (rc != SQLITE_OK)
    {
        throw cRuntimeError("cSQLiteOutputScalarManager:: Could not prepare statement (SQL_INSERT_STATISTIC_ATTR): %s",
                sqlite3_errmsg(connection));
    }
    rc = sqlite3_prepare_v2(connection, SQL_INSERT_FIELD, strlen(SQL_INSERT_FIELD), &insertFieldStmt, 0);
    if (rc != SQLITE_OK)
    {
        throw cRuntimeError("cSQLiteOutputScalarManager:: Could not prepare statement (SQL_INSERT_FIELD): %s",
                sqlite3_errmsg(connection));
    }
    rc = sqlite3_prepare_v2(connection, SQL_INSERT_BIN, strlen(SQL_INSERT_BIN), &insertBinStmt, 0);
    if (rc != SQLITE_OK)
    {
        throw cRuntimeError("cSQLiteOutputScalarManager:: Could not prepare statement (SQL_INSERT_BIN): %s",
                sqlite3_errmsg(connection));
    }
}
...
void cSQLiteOutputScalarManager::insertBin(size_t statisticId, double binlowerbound, size_t value)
{
    ...
    if (rc != SQLITE_OK)
    {
        throw cRuntimeError("cSQLiteOutputScalarManager:: Could not bind binlowerbound.");
    }
    ...
    if (rc != SQLITE_DONE)
    {
        throw cRuntimeError("cSQLiteOutputScalarManager:: Could not execute statement (SQL_INSERT_BIN): %s",
                sqlite3_errmsg(connection));
    }
